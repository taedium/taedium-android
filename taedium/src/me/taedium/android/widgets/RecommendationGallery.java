...
public class RecommendationGallery extends Gallery {
    ...
    public RecommendationGallery(Context context) {
        super(context);
        final float scale = context.getResources().getDisplayMetrics().density;
        mDragBoundsInPx = (int) (scale*DRAG_BOUNDS_IN_DP + 0.5f);
    }
    
    public RecommendationGallery(Context context, AttributeSet attrs) {
        super(context, attrs);
        final float scale = context.getResources().getDisplayMetrics().density;
        mDragBoundsInPx = (int) (scale*DRAG_BOUNDS_IN_DP + 0.5f);
    }
    ...
    /**
     * Overriding this method is what allows to swipe only one recommendation at a time
     */
    @Override
